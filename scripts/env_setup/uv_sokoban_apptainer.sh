...
# Install the Sokoban uv environment inside Apptainer,
...
AGENTENV_SOKOBAN="$PROJECT_ROOT/AgentGym/agentenv-sokoban"
...
if [ ! -d "$AGENTENV_SOKOBAN" ]; then
  echo "Error: Expected directory not found: $AGENTENV_SOKOBAN" >&2
  exit 1
fi
...
UV_BIN=\"$UV_BIN\"
VENV_DIR=\"$VENV_DIR\"
AGENTENV_SOKOBAN=\"$AGENTENV_SOKOBAN\"
...
# gym_sokoban imports pkg_resources, which is removed in setuptools>=81.
\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" \"setuptools<81\"
\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" -e \"\$AGENTENV_SOKOBAN\"
\"\$UV_BIN\" pip list -p \"\$VENV_DIR/bin/python\" | grep -E \"(agentenv_sokoban|fastapi|uvicorn|gym|gym-sokoban|numpy|matplotlib)\" || true
"
