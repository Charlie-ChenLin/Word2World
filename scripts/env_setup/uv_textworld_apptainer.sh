...
# Install the TextWorld uv environment inside Apptainer,
...
ENV_NAME="${ENV_NAME:-uv_textworld}"
...
AGENTENV_TEXTWORLD="$PROJECT_ROOT/AgentGym/agentenv-textworld"
TEXTWORLD_SRC="$AGENTENV_TEXTWORLD/textworld"
...
if [ ! -d "$AGENTENV_TEXTWORLD" ]; then
  echo "Error: Expected directory not found: $AGENTENV_TEXTWORLD" >&2
  exit 1
fi
...
UV_BIN=\"$UV_BIN\"
VENV_DIR=\"$VENV_DIR\"
AGENTENV_TEXTWORLD=\"$AGENTENV_TEXTWORLD\"
TEXTWORLD_SRC=\"$TEXTWORLD_SRC\"
...
if [ -d \"\$TEXTWORLD_SRC\" ]; then
  echo \"Installing local TextWorld clone...\"
  \"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" -e \"\$TEXTWORLD_SRC\"
else
  echo \"Skipping local TextWorld clone: not found at \$TEXTWORLD_SRC\"
fi

\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" -e \"\$AGENTENV_TEXTWORLD\"
\"\$UV_BIN\" pip list -p \"\$VENV_DIR/bin/python\" | grep -E \"(textworld|fastapi|uvicorn|agentenv_textworld)\" || true
"
