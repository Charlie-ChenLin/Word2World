...
# Install the SciWorld uv environment inside Apptainer,
...
SCIWORLD_DIR="$PROJECT_ROOT/AgentGym/agentenv-sciworld"
...
if [ ! -d "$SCIWORLD_DIR" ]; then
  echo "Error: Expected directory not found: $SCIWORLD_DIR" >&2
  exit 1
fi
...
if [ \"\${VENV_CLEAR:-0}\" = \"1\" ]; then
  \"\$UV_BIN\" venv \"\$VENV_DIR\" --python 3.8 --clear
else
  \"\$UV_BIN\" venv \"\$VENV_DIR\" --python 3.8
fi

\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" --upgrade pip
\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" -e \"\$SCIWORLD_DIR\"
