...
# Install the BabyAI uv environment inside Apptainer,
...
AGENTENV_BABYAI="$PROJECT_ROOT/AgentGym/agentenv-babyai"
...
if [ ! -d "$AGENTENV_BABYAI" ]; then
  echo "Error: Expected directory not found: $AGENTENV_BABYAI" >&2
  exit 1
fi
...
UV_BIN=\"$UV_BIN\"
VENV_DIR=\"$VENV_DIR\"
AGENTENV_BABYAI=\"$AGENTENV_BABYAI\"
...
\"\$UV_BIN\" pip install -p \"\$VENV_DIR/bin/python\" -e \"\$AGENTENV_BABYAI\"
\"\$UV_BIN\" pip list -p \"\$VENV_DIR/bin/python\" | grep -E \"(agentenv_babyai|fastapi|uvicorn|gymnasium|minigrid)\" || true
"
