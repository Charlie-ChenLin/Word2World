...
class RolloutHandler:
    def __init__(
        ...
        env_mask: Optional[List[int]] = None,
        ...
        self.env_mask = env_mask if env_mask is not None else [0] * len(input_ids)
    ...
    def add_assistant_message(
        ...
        _env_mask = [0] * len(append_token_ids)
        ...
        assert len(self.input_ids) == len(self.attention_mask) == len(self.position_ids) == len(self.loss_mask) == len(self.env_mask), f"""Rollout Handler has different length of {len(self.input_ids)=}, 
            {len(self.attention_mask)=}, {len(self.position_ids)=}, {len(self.loss_mask)=}, {len(self.env_mask)=}"""
    ...
    def add_user_message(
        ...
        _env_mask += [0] * len(suffix_token_ids)
        self.input_ids += append_token_ids
        _attention_mask = [1] * len(append_token_ids)
        self.attention_mask += _attention_mask
        _delta_position_ids = [pos_id for pos_id in range(1, len(append_token_ids) + 1)]
